...
def evaluate_pipeline(test_df, hmm, bn_model, vocab, obs_dict):
    ...
    for idx in tqdm(range(0, len(test_df) - sequence_length)):
        ...
        # Prepare HMM sequence
        sequence = [f"{row['activity']}@{row['location']}" for _, row in seq_slice.iterrows()]
        encoded_seq = np.array([obs_dict.get(o, 0) for o in sequence[:-1]])  # exclude last, predict next
        ...
        # Combine HMM and BN predictions
        hmm_distribution = {}
        ...
        # Prediction: pick highest probability state
        if combined_probs:
            predicted_state = max(combined_probs.items(), key=lambda x: x[1])[0]
        else:
            predicted_state = None  # fallback
