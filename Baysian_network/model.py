...
class HiddenMarkovModel:
    ...
    def backward(self, obs_seq):
        """
        Backward algorithm: Computes probability of reaching final state from given state.
        """
        T = len(obs_seq)
    ...
    def predict(self, obs_seq):
        """
        Predict the most likely sequence of hidden states given an observation sequence using Viterbi Algorithm.
        """
        T = len(obs_seq)
        delta = np.zeros((T, self.n_states))  # Stores max probability
        psi = np.zeros((T, self.n_states), dtype=int)  # Stores back-pointers
        ...
        states_seq[-1] = np.argmax(delta[-1])  # Start from last state
        for t in range(T-2, -1, -1):
